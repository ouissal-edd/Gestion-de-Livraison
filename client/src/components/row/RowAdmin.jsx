...
export const RowAdmin = ({ managersData, getManager }) => {
    ...
    // Update
    const PUT = async (id) => {
        await axios.put(`http://localhost:5000/admin/updateManager/${id}`, {
            fullName: fullName,
            email: email,
        })
            .then(res => getManager())
        ...
        <>
            {
                managersData.map(manager => (
                    <>
                        <tr key={manager._id} className="bg-white border-b-2 border-gray-200 text-center">
                            ...
                            <td className="px-16 py-2 text-center ">
                                <button className='p-2' onClick={() => setDeleteModal(true, setIdu(manager._id))}>
                                    <TiUserDelete />
                                </button>
                                <button className='p-2 ' onClick={() => setShowModal(true, setNom(manager.fullName), setEmail(manager.email), setIdu(manager._id))}>
                                    <AiFillEdit />
                                </button>
