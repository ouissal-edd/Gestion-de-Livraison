...
const Login = async (req, res) => {
    ...
    try {
        ...
        const passwordCorrect = await bcrypt.compare(password, existingChauffeur.password)
        if (!passwordCorrect)
            return res.status(401).json({
                errorMessage: "Wrong email or password"
            })
        ...
        res.cookie('role', existingChauffeur.role, {
            httpOnly: true
        }).send()
...
const takeDelivery = async (req, res) => {
    try {
        ...
        const livraison = await Chauffeur.findOneAndUpdate({
            _id: Chauffeur_id
        }, {
            $push: {
                livraisons: req.body.ref_livraison
            }
        });
        const Disable_livraison = await Livraison.findOneAndUpdate({
            _id: req.body.ref_livraison
        }, {
            $set: {
        ...
        return res.status(200).json({
            success: "this delivery is affected to u :)"
        })
...
const Prime = async (req, res) => {
    try {
        ...
        // prendre la livraison affectée 
        const chauffeur = await Chauffeur.findById({
            _id: Chauffeur_id
        })

        await Promise.all(chauffeur.livraisons.map(async (element) => {
            console.log(element)
            const livraison = await Livraison.findById({
                _id: element
            })
        ...
        if (totaleDistance === 1000) {
            prime = prix * 0.15
            const result = await Chauffeur.updateOne({
                _id: Chauffeur_id
            }, {
                prime: prime
            });
...
const getLivraisonByType = async (req, res) => {
    try {
